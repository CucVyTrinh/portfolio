...
function DisplayStandModel({ posterId = 1, position = [0, -50, 0] }) {
  const group = useRef();
  ...
  return (
    <group ref={group} position={position} rotation={[0, Math.PI / 4, 0]} scale={0.5}>
      <primitive object={clone} />
    </group>
  );
}

const STAND_SPACING = 120;

function AllThreeStands() {
  return (
    <>
      <DisplayStandModel posterId={1} position={[-STAND_SPACING, -50, 0]} />
      <DisplayStandModel posterId={2} position={[0, -50, 0]} />
      <DisplayStandModel posterId={3} position={[STAND_SPACING, -50, 0]} />
    </>
  );
}

export function DisplayStand3DRow() {
  return (
    <div
      style={{
        width: "100%",
    ...
    >
      <Canvas
        frameloop="always"
        camera={{ position: [0, 0, 180], fov: 54 }}

 ...
 * -----------------------
 * Distortion layer for the fixed header: applies liquid/refraction effect to
 * the *background content* visible through the header only. Header UI stays sharp.
 *
 * Implementation: WebGL fragment shader (alternative: feTurbulence + feDisplacementMap).
 * - Captures the page strip behind the header into a texture (throttled on scroll/resize).
 * - Shader: turbulence/noise-based displacement, horizontal + vertical warping,
 *   animated flow (slow, smooth); chroma offset for subtle color fringing.
 * - prefers-reduced-motion: animation stops (static refraction).
 */
export default function LiquidDistortionBackdrop({
  ...
  // Shader source: vertex = full-screen quad; fragment = turbulence + displacement.
  const shaderSource = useMemo(() => {
    // Vertex: full-screen quad; UVs for texture sampling.
    const vs = `
      attribute vec2 aPos;
      varying vec2 vUv;
      void main() {
    ...
    /**
     * Fragment shader: liquid refraction (turbulence + displacement).
     * - fbm() = fractal Brownian motion (noise); horizontal + vertical warping.
     * - uTime * uFlow = slow ambient motion (not tied to scroll).
     * - Chroma offset (duv +/- chroma for R/G/B) = subtle color fringing.
     */
    const fs = `
  ...
  useEffect(() => {
    ...
    // Listen for prefers-reduced-motion changes at runtime.
    const mq =
    ...
    const draw = (tMs) => {
      ...
      gl.uniform1f(uAmp, 0.042);  /* displacement strength: bend text/shapes underneath */
      gl.uniform1f(uFlow, 0.35);  /* flow speed: slow, smooth ambient motion */

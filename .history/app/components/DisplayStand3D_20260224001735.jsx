...
const INITIAL_Y_ANGLE = Math.PI / 4; // 45° – same starting angle for all stands
const TILT_CLAMP = 0.35; // limit vertical tilt (radians) so drag feels natural
...
function DisplayStandModel({ posterId = 1 }) {
  ...
  return (
    <group ref={group} position={[0, -50, 0]} rotation={[0, 0, 0]} scale={0.5}>
...
/** Wraps a stand so it can be rotated by dragging. Uses document listeners so drag continues if pointer leaves canvas. */
function DraggableStand({ position, children }) {
  ...
  useEffect(() => {
    ...
    const handleMove = (e) => {
      ...
      groupRef.current.rotation.y += e.movementX * ROTATE_SENSITIVITY;
      const tilt = groupRef.current.rotation.x + e.movementY * ROTATE_SENSITIVITY;
      groupRef.current.rotation.x = THREE.MathUtils.clamp(tilt, -TILT_CLAMP, TILT_CLAMP);
    };
    const handleUp = () => {
      setDragging(false);
...
function ThreeStandsScene() {
  return (
    <>
      ...
      <DraggableStand position={[-STAND_OFFSET_X, 0, 0]}>
        <DisplayStandModel posterId={1} />
      </DraggableStand>
      <DraggableStand position={[0, 0, 0]}>

...
const POSTER_PATH = "/project/Reijin/poster-1.jpg";

function DisplayStandModel() {
  const group = useRef();
  const obj = useLoader(OBJLoader, OBJ_PATH);
  const posterTexture = useTexture(POSTER_PATH);
  ...
  useEffect(() => {
    ...
    posterTexture.repeat.set(1.2, 1.2);
    posterTexture.offset.set(-0.1, -0.1);
    ...
    obj.traverse((child) => {
      ...
      if (isSurface && child.material) {
        ...
        if (geom?.getIndex && geom.getAttribute("position")) {
          ...
          if (index && uvAttr) {
            ...
            for (let i = 0; i < index.count; i += 3) {
              ...
              normal.crossVectors(v1.clone().sub(v0), v2.clone().sub(v0)).normalize();
              const keep = useFront ? normal.z > 0.1 : normal.z < -0.1;
              if (keep) {

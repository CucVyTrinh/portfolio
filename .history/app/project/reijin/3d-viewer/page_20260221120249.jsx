...
function DisplayStandModel() {
  ...
  return (
    <group ref={group} rotation={[0, Math.PI / 4, 0]} scale={0.35}>
...
export default function Reijin3DViewerPage() {
  useEffect(() => {
    ...
    if (pageWrap) pageWrap.style.background = bg;
    return () => {
      document.documentElement.style.background = "";
      document.body.style.background = "";
      const el = document.querySelector(".pageWrap");
  ...
  return (
    ...
    >
      <Canvas
        camera={{ position: [0, 0, 80], fov: 44 }}
        gl={{ antialias: true, alpha: true }}
        onCreated={({ gl }) => {
          gl.setClearColor(0x121010, 1);
        }}

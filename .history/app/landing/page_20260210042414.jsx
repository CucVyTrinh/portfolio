...
export default function LandingPage() {
  ...
  const [pillTooltipDisplayed, setPillTooltipDisplayed] = useState("");
  const timeoutRef = useRef(null);
  const intervalRef = useRef(null);
  const tooltipIntervalRef = useRef(null);
  const pillTooltipIntervalRef = useRef(null);
  ...
  /* Typewriter effect for "[View Projects]" when hovering a pill */
  useEffect(() => {
    if (pillTooltipIntervalRef.current) {
      clearInterval(pillTooltipIntervalRef.current);
      pillTooltipIntervalRef.current = null;
    }
    if (!pillTooltip) {
      setPillTooltipDisplayed("");
      return;
    }
    const fullText = "[View Projects]";
    setPillTooltipDisplayed("");
    let i = 0;
    pillTooltipIntervalRef.current = setInterval(() => {
      if (i < fullText.length) {
        setPillTooltipDisplayed(fullText.slice(0, i + 1));
        i++;
      } else {
        clearInterval(pillTooltipIntervalRef.current);
        pillTooltipIntervalRef.current = null;
        ...
        <div className={styles.mainContentWrap}>
          ...
          >
            ...
            {pillTooltip && (
              ...
              >
                {pillTooltipDisplayed}
                <span className={styles.aboutPillTooltipCursor} aria-hidden>|</span>
              </span>
            )}
          </section>

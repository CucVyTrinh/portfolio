...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const fullText =
      tooltip.label === "viewProjects" ? "[View Projects]" : `[${capitalize(tooltip.label)}]`;
    setTooltipDisplayed("");
    let i = 0;
      ...
      <main>
        ...
        <div className={styles.mainContentWrap}>
          ...
          <section id="about" className={styles.about}>
            <span className={styles.aboutLineLeft} aria-hidden />
            <p
              className={styles.aboutText}
              onMouseLeave={() =>
                setTooltip((prev) => (prev?.label === "viewProjects" ? null : prev))
              }
            >
              Specializing in{" "}
              <Link
                href="/project"
                className={styles.highlightTerm}
                onMouseEnter={(e) =>
                  setTooltip({ label: "viewProjects", x: e.clientX, y: e.clientY })
                }
                onMouseMove={(e) =>
                  setTooltip((prev) =>
                    prev?.label === "viewProjects"
                      ? { ...prev, x: e.clientX, y: e.clientY }
                      : prev
                  )
                }
            ...
            <span className={styles.aboutLineRight} aria-hidden />
          </section>
